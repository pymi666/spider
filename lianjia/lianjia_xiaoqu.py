...
class LianjiaXiaoqu:
    ...
    def get_jiedao_url(self):
        ...
        for daqu,jiedao_list in self.jiedao_json.items():
            for jiedao_dict in jiedao_list:
                ...
                jiedao_url_all.add(jiedao_url)
        #print (len(jiedao_url_all))
    ...
    def get_xiaoqu(self,html_str):
        ...
        for a in daqu_list:
            ...
            item["街道"] = a.xpath("//div[@class='positionInfo']/a[2]/text()")[0]
            item["大区"] = a.xpath("//div[@class='positionInfo']/a[1]/text()")[0]
            item["出租"]=int(list(a.xpath(".//div[@class='houseInfo']/a[2]/text()")[0])[0])
            item["挂牌"]=int(a.xpath(".//a[@class='totalSellCount']/span/text()")[0])
            SQL = "INSERT INTO xiaoqu2 (小区,均价,建设时间,出租数,挂牌数,街道,大区) VALUES ('%s',%d,'%s',%d,%d,'%s','%s')"%(item['小区'],item['均价'],item['建筑时间'],item['出租'],item['挂牌'],item['街道'],item['大区'])
            print (SQL)
            self.save_sql(SQL)
            print (item)
    def for_xiaoqu(self,index,url):

        for i in range(1,index+1):
            index_url = url+"pg"+str(i)
            html=self.parse_url(url) if i==1 else self.parse_url(index_url)
            self.get_xiaoqu(html)
